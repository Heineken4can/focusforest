...
export class SyncRateLimitService {
  ...
  async assertWithinLimit(bucket: string, identity: string): Promise<void> {
    const max = this.configService.get<number>('rateLimit.sync.max') ?? 100;
    const windowSec =
      this.configService.get<number>('rateLimit.sync.windowSec') ?? 60;
  ...
  private buildKey(bucket: string, identity: string): string {
    return `rate-limit:sync:${bucket}:${identity}`;
  }
}
