...
| v1.3 | 2026-03-12 | UI-Plan | UI rv 반영: signup 필수 필드, stale TBD, CSRF 계약 정리 |
...
| 추가 메모 | 리스트의 `보조 설명`은 `Task.description` optional field를 그대로 매핑한다. V1에서는 카테고리, 일정, 최근 수정 시각을 이 슬롯의 대체값으로 사용하지 않는다. |
...
| 주요 액션/API 후보 | `signUp(email, password, displayName, timezone?)`, `logIn(email, password)`, `enterLocalMode`, `bootstrapAfterLogin`, `migrateLocalDataToServer`, `queueKpiEvent(app_first_open, 최초 진입 또는 신규 설치 후 첫 앱 구동 시 1회)` |
| 서버 저장 사실값 | `User`, `AuthSession`, 로그인 후 `Task/FocusSession/RewardGrant/UserSetting` 마이그레이션 결과 |
| 로컬/런타임 상태 | 폼 입력값(`email`, `password`, `displayName(signup only)`), 필드 에러, 비밀번호 표시/숨김, 로그인 로딩, 로컬 준비중 상태 |
...
| 추가 메모 | 토큰 재발급은 `auth/refresh`와 CSRF double-submit 검증을 따른다. `signup`, `login`, `refresh` 성공 시 서버는 non-HttpOnly `csrfToken` cookie를 설정/회전하고, 클라이언트는 이 값을 `X-CSRF-Token` 헤더로 반영한다. 로그인 직후 로컬 데이터 연결은 `deviceId` + `clientGeneratedId` 기준 bootstrap 병합으로 처리한다. `app_first_open`은 device/install scope dedupeKey로 1회만 적재한다. |
...
- 대시보드 리스트의 `보조 설명` 필드는 `Task.description`을 사용한다. category/date/updatedAt 대체 슬롯은 V1 범위에 포함하지 않는다.
...
- `signup`, `login`, `refresh` 성공 시 서버는 non-HttpOnly `csrfToken` cookie를 발급/회전하고, 클라이언트는 `auth/refresh`, `auth/logout` 요청에 `X-CSRF-Token` 헤더로 이 값을 전달한다.
